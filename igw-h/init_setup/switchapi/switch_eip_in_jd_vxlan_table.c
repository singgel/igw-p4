...
static void eip_in_entry_init() {
	uint32_t srcip;
	eipInJdVxlanKey key;

	memset(&key, 0, sizeof(eipInJdVxlanKey));
	...
	assert(entry_add_with_rewrite_eip_in_jd_vxlan(&key,srcip) == 0);
...
static void az_in_entry_init() {
	...
	assert(entry_add_with_rewrite_az_in_jd_vxlan(&key,shared_bw_vip) == 0);
}
