...
const bf_rt_table_hdl *ecmp_group_selector_Table = NULL;
...
// during init time.
void ecmp_group_selector_table_setup() {
	bf_status_t bf_status;
	jd_bf_rt_t *jd_bf_p = &jd_bfrt;
	
  	// Get table object from name
  	bf_status = bf_rt_table_from_name_get(jd_bf_p->bfrtInfo, 
  					"P02_Ingress.ecmp_group.pipe02_group_selector", 
  					&ecmp_group_selector_Table);
  	assert(bf_status == BF_SUCCESS);

  	// Get key Ids 
  	bf_status = bf_rt_key_field_id_get(ecmp_group_selector_Table, 
  					"$SELECTOR_GROUP_ID", &selector_key_field_id);
  	assert(bf_status == BF_SUCCESS);
	...
	bf_status = bf_rt_data_field_id_get(
  	...
  	bf_status = bf_rt_table_key_allocate(ecmp_group_selector_Table, &bfrtTableKey);
  	assert(bf_status == BF_SUCCESS);
  	bf_status = bf_rt_table_data_allocate(ecmp_group_selector_Table, &bfrtTableData);
  	assert(bf_status == BF_SUCCESS);
}

static int ecmp_group_selector_key_setup(ecmpGroupSelectorKey *key,
...
static int ecmp_group_selector_data_setup(bf_rt_table_data_hdl *table_data,
...
int ecmp_group_selector_entry_add(ecmpGroupSelectorKey *key,
	...
	jd_bf_rt_t *jd_bf_p = &jd_bfrt;
  	...
  	bf_rt_table_key_reset(ecmp_group_selector_Table, &bfrtTableKey);
  	bf_rt_table_data_reset(ecmp_group_selector_Table, &bfrtTableData);

  	// Fill in the Key and Data object
  	if (ecmp_group_selector_key_setup(key, bfrtTableKey) < 0)
		return -1;
  	if(ecmp_group_selector_data_setup(bfrtTableData,max_group_size,memberid_val,
