...
const bf_rt_table_hdl *eip_in_selector_Table = NULL;
...
// during init time.
void eip_in_selector_table_setup() {
	bf_status_t bf_status;
	jd_bf_rt_t *jd_bf_p = &jd_bfrt;
	
  	// Get table object from name
  	bf_status = bf_rt_table_from_name_get(jd_bf_p->bfrtInfo, "P13_Egress.eip_in_redirect.dl_ip_group_selector", 
  				&eip_in_selector_Table);
  	assert(bf_status == BF_SUCCESS);

  	// Get key Ids 
  	bf_status = bf_rt_key_field_id_get(eip_in_selector_Table, "$SELECTOR_GROUP_ID", 
	...
	bf_status = bf_rt_data_field_id_get(
  	...
  	bf_status = bf_rt_table_key_allocate(eip_in_selector_Table, &bfrtTableKey);
  	assert(bf_status == BF_SUCCESS);
  	bf_status = bf_rt_table_data_allocate(eip_in_selector_Table, &bfrtTableData);
  	assert(bf_status == BF_SUCCESS);
}

static int eip_in_selector_key_setup(const eipInSelectorKey *key,
...
static int eip_in_selector_data_setup(bf_rt_table_data_hdl *table_data,
...
int eip_in_selector_entry_add(const eipInSelectorKey *key,
                                        uint16_t max_group_size,
                                        uint32_t *memberid_val,uint16_t memberid_num,
										bool *memberstatus_val,uint16_t memberstatus_num,
										const bool add) {
	...
	jd_bf_rt_t *jd_bf_p = &jd_bfrt;
  	...
  	bf_rt_table_key_reset(eip_in_selector_Table, &bfrtTableKey);
  	bf_rt_table_data_reset(eip_in_selector_Table, &bfrtTableData);

  	// Fill in the Key and Data object
  	if (eip_in_selector_key_setup(key, bfrtTableKey) < 0)
		return -1;
  	if(eip_in_selector_data_setup(bfrtTableData,max_group_size,memberid_val,
  	...
  	status = BF_SUCCESS;
  	if (add) {
    	status = bf_rt_table_entry_add(eip_in_selector_Table, 
					jd_bf_p->session, 
    				&jd_bf_p->dev_tgt, 
    	...
    	status = bf_rt_table_entry_mod(eip_in_selector_Table, 
					jd_bf_p->session, 
    				&jd_bf_p->dev_tgt,
    		#ifdef BFRT_GENERIC_FLAGS
							       0,
